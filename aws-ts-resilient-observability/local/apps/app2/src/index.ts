...
async function initKafkaConsumer(): Promise<void> {
  ...
  console.log(`Kafka consumer initialized, subscribed to topic: ${KAFKA_TOPIC}`);
}
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      console.log(`App2 (Consumer) health server listening on port ${PORT}`);
    ...
    console.log('App2 (Consumer) is running and processing messages');
